...
describe('main window', function spec() {
  ...
  const getMainTitle = () => this.driver.findElement(webdriver.By.className('title'));

  it('should open window', async () => {
    const title = await this.driver.getTitle();
    expect(title).to.equal('Build Checker App');
  });

  // it('Tray menu validation', async () => {
  //   console.log('route:configurations');
  //
  //   await ipcRenderer("route:configurations");
  //
  //   await this.driver.executeScript(command);
  //   const link = await getMainTitle();
  //   expect(await link.getText()).to.equal('Configurations');
  // });
